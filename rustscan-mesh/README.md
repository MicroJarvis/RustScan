...
- `cargo test -p rustscan-mesh --lib --quiet`: `210 passed; 0 failed`
...
env RUSTFLAGS=-Awarnings cargo run --manifest-path rustscan-mesh/Cargo.toml --release --example openmesh_compare_normals --quiet
...
```bash
# Build
cargo build --manifest-path rustscan-mesh/Cargo.toml --release

# Full library test suite
cargo test --manifest-path rustscan-mesh/Cargo.toml --lib

# Focused RustMesh areas
cargo test --manifest-path rustscan-mesh/Cargo.toml --lib tools::decimation::tests
cargo test --manifest-path rustscan-mesh/Cargo.toml --lib tools::remeshing::tests
cargo test --manifest-path rustscan-mesh/Cargo.toml --lib tools::vdpm::tests
