...
fn main() -> Result<()> {
    ...
    // A genuinely partial roots workgroup, unlike either timed batch size.
    for profile in [false, true] {
        let (results, _, _) = replay(&diagnostic, &inputs, 32767, profile)?;
        ensure!(
            signature(&results) == PREVIOUS_SIGNATURE,
            "partial batch signature changed"
        );
    }
    ...
    for batch in [32768, 65024] {
        ...
        for round in 0..2 {
            ...
            for step in 0..3 {
                ...
                let record = json!({"round":round%1,"mode":mode,"seconds":elapsed,"signature":sig,"chunks":timings});
                if round < 1 {
                    warmups.push(record);
                } else {
    ...
    let representatives: Vec<_> = ids
        ...
        .collect::<Result<_>>()?;
    let report = json!({"schema":"five-point-roots-layout-v1","partial_batch_32767_signatures_match":true,"database":args.database,
        ...
        "warmup_rounds_per_mode_per_batch":1,"measured_rounds_per_mode_per_batch":1,
        ...
        "limitations":["One adapter, one measured round per process; compare at least three interleaved baseline/candidate processes. No statistical performance guarantee or production equivalence.",
            ...
            "Only roots execution layout may differ between binaries; thresholds and matching/RANSAC unchanged. CPU calculations are offline diagnostics only."]});
...
mod tests {
    use super::*;
    #[test]
    fn actual_gpu_roots_partial_workgroups() -> Result<()> {
        ...
        for c in &polynomials {
            let mut r = gpu.compute_polynomial_roots(&[*c])?.remove(0);
            r.trial = expected.len();
            expected.push(r);
        }
        for count in [1, 2, 31, 32, 33, 63, 64, 65, 127, 128, 129] {
            let actual = gpu.compute_polynomial_roots(&polynomials[..count])?;
