...
- RustViewer 维护导入媒体、运行 RustSFM、消费 COLMAP 产物并衔接 RustGS 训练的桌面工作流；macOS 新项目默认使用并行 CPU VLFeat SIFT（60 图实测 23.3s / 638,895 特征，对比当前 wgpu SIFT 59.6s / 424,554 特征），既有 manifest 保留用户已保存的 GPU 选择。
