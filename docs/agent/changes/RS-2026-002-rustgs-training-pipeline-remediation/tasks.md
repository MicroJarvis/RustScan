...
- **P1 scan 所有权缺口：** rustscan-gs/src/training/gpu_primitives/prefix_sum.rs:371 的训练路径每次仍调用 empty_tensor 创建输出。workspace 只复用递归 scratch，step_fresh_allocations 没有统计该输出分配；prefix_sum.rs:291 与 trainer.rs:720 的 TLS raw pointer 绑定跨越 await，在多线程异步 runtime 下不安全。
...
- **P2 格式门禁失败：** cargo fmt --package rustscan-gs --check 当前仍报告 rustscan-gs/src/lib.rs、rustscan-gs/src/training/engine/trainer.rs、rustscan-gs/src/training/forward/parity.rs、rustscan-gs/src/training/forward/sorting.rs 差异。
...
- 只修改 RustGS 相关文件和本计划；不得带入 rustscan-sfm/rustscan-viewer 未提交修改。
...
**文件：** 修改 rustscan-gs/src/training/forward/dispatch.rs、forward/mod.rs、forward/shaders/write_dispatch.wgsl、engine/trainer.rs、engine/loss.rs、backward/autodiff.rs；测试 rustscan-gs/tests/bounded_forward_parity.rs、rustscan-gs/tests/checkpoint_resume.rs 及 engine GPU fixtures。
...
**文件：** rustscan-gs/src/training/engine/trainer.rs、engine/runtime.rs、reporting/telemetry.rs。
...
**文件：** rustscan-gs/src/training/gpu_primitives/prefix_sum.rs、forward/tile_mapping.rs、forward/mod.rs、engine/trainer.rs。
...
**文件：** rustscan-gs/src/training/gpu_primitives/prefix_sum.rs、training/reporting/telemetry.rs、engine/trainer.rs。
...
**文件：** 新建 rustscan-gs/src/training/reporting/gpu_profiler.rs；修改 training/reporting/mod.rs、telemetry.rs、optimization_report.rs、engine/runtime.rs；测试 gpu_profiler.rs 单元测试和 report JSON fixtures。
...
**文件：** 新建 rustscan-gs/src/training/evaluation/split.rs；修改 rustscan-gs/src/bin/rustgs/train_command.rs、training/evaluation/mod.rs、optimization_report.rs、comparator；测试 split manifest unit/integration tests。
...
**文件：** rustscan-gs/src/training/backward/gradient_check.rs、rustscan-gs/tests/gradient_boundary.rs。
...
**文件：** rustscan-gs/src/training/config.rs、engine/trainer.rs、training/checkpoint.rs、comparator。
...
**文件：** rustscan-gs/src/lib.rs、rustscan-gs/src/training/engine/trainer.rs、rustscan-gs/src/training/forward/parity.rs、rustscan-gs/src/training/forward/sorting.rs，以及本计划和对应 review 记录。

- [ ] 运行 cargo fmt --package rustscan-gs，只接受 rustfmt 产生的格式变更，不顺手改行为。
- [ ] 用 git diff --check 检查空白；确认 git status 不含 rustscan-sfm/rustscan-viewer 变更的 staged/committed 混入。
...
在 P0–P2 任务完成后执行：
    ...
    cargo test -p rustscan-gs --test checkpoint_resume --features gpu-wgpu -- --test-threads=1
    git diff --check
