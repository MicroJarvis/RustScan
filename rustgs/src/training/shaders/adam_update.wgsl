...
@group(0) @binding(5) var<storage, read> status: array<u32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // word 5 = mutation gate; word 4 = committed optimizer steps after prepare.
    if (status[5] == 0u) {
        return;
    }
    let step = max(status[4], 1u);
    ...
    let bias_correction1 = 1.0 - pow(params.beta1, f32(step));
    let bias_correction2 = 1.0 - pow(params.beta2, f32(step));
