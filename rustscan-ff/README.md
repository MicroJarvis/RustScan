...
```bash
cargo test -p rustscan-ff --lib
cargo test -p rustscan-ff --lib --features onnx-ort
cargo check -p rustscan-ff --features onnx-candle
```
...
Model export helpers live in `rustscan-ff/scripts/`. Exported models are not stored
