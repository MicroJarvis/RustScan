...
pub struct ReconstructionOutput {
    ...
    pub summary: rustscan_sfm::ReconstructionSummary,
}
...
impl ReconstructionRunner for RustSfmRunner {
    fn run(
        ...
        let summary = rustscan_sfm::run_reconstruction_with_callbacks(&config, Some(&mut sink))?;
...
pub fn mapper_config_for(input: &Path, output: &Path) -> rustscan_sfm::MapperConfig {
    rustscan_sfm::MapperConfig {
        ...
        copy_images: true,
        ..rustscan_sfm::MapperConfig::default()
    }
}

pub fn validate_completed_summary(
    summary: &rustscan_sfm::ReconstructionSummary,
) -> anyhow::Result<()> {
    ensure!(
...
impl rustscan_sfm::PipelineCallbackSink for ProgressSink<'_> {
    fn on_pipeline_callback(&mut self, event: &rustscan_sfm::PipelineCallbackEvent) {
...
mod tests {
    ...
    #[test]
    fn mapper_config_enables_isolated_shared_camera_local_matching_and_image_copying() {
    ...
    fn completed_summary_accepts_registered_sparse_model() {
        let complete = rustscan_sfm::ReconstructionSummary {
            images: 12,
            registered_images: 8,
            points: 120,
            pairs: 24,
