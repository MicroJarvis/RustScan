...
/// ```no_run
/// use std::path::Path;
/// use rustscan_viewer::loader::gaussian::load_gaussians;
/// use rustscan_viewer::renderer::scene::Scene;
///
...
pub fn load_gaussians_with_splats(
    path: &Path,
    scene: &mut Scene,
) -> Result<Option<rustscan_gs::HostSplats>, LoadError> {
    match rustscan_gs::load_splats(path) {
        Ok((splats, _meta)) => {
...
fn append_rustgs_splats(splats: &rustscan_gs::HostSplats, scene: &mut Scene) {
...
fn should_try_legacy_rustslam_loader(path: &Path, err: &rustscan_gs::SceneIoError) -> bool {
    if path
        .extension()
    ...
    match err {
        rustscan_gs::SceneIoError::InvalidFormat { message } => {
...
fn load_legacy_rustslam_gaussians(
    path: &Path,
) -> Result<Vec<rustscan_slam::fusion::Gaussian>, LoadError> {
    rustscan_slam::fusion::load_scene_ply(path)
...
fn legacy_gaussians_to_splats(
    gaussians: &[rustscan_slam::fusion::Gaussian],
) -> Result<rustscan_gs::HostSplats, LoadError> {
    ...
    rustscan_gs::HostSplats::from_components(
        positions,
        log_scales,
        rotations,
        opacity_logits,
...
mod tests {
    use super::*;
    use rustscan_slam::fusion::{save_scene_ply, Gaussian, SceneMetadata};
    ...
    fn loads_rustgs_training_ply_layout() {
        ...
        let tmp = tempfile::NamedTempFile::with_suffix(".ply").unwrap();
        let splats = rustscan_gs::HostSplats::from_components(
            vec![1.0, 2.0, 3.0],
