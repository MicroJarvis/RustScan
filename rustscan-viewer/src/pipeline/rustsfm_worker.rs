...
struct ImportedSequence {
    frames: Vec<rustscan_sfm::SequenceFrame>,
}
...
struct KeyframeStageResult {
    ...
    selection_config: Option<rustscan_sfm::AdaptiveKeyframeSelectionConfig>,
    evaluated_pairs: usize,
    diagnostics: Vec<rustscan_sfm::AdaptiveKeyframePairDiagnostic>,
}
...
impl SfmWorker for RustSfmWorker {
    fn run(
        ...
        let mut task = rustscan_sfm::SfmTaskContext::new(&sfm_control, &mut task_sink);
        let selection = resolve_keyframe_selection_with(
            ...
            &output,
            &mut task,
            rustscan_sfm::run_adaptive_keyframe_selection,
        );
        ...
        let result = rustscan_sfm::run_keyframe_reconstruction(
            &sequence.frames,
...
impl PnpWorker for RustSfmWorker {
    fn run(
        ...
        let result = run_remaining_registration_with(
            ...
            &output,
            &mut task,
            rustscan_sfm::register_remaining_sequence_frames,
        );
        drop(task);
        drop(task_sink);
...
fn mapper_config_for(request: &StageRequest) -> rustscan_sfm::MapperConfig {
    let mut mapper_config = rustscan_sfm::MapperConfig::default();
    ...
    mapper_config.matching_pair_strategy =
        rustscan_sfm::MatchingPairStrategy::LocalWindow { window: 5 };
...
fn registration_config_for(request: &StageRequest) -> rustscan_sfm::SequenceRegistrationConfig {
    let pnp_config = &request.manifest.pnp_config;
    rustscan_sfm::SequenceRegistrationConfig {
...
fn resolve_keyframe_selection_with<F>(
    mode: KeyframeSelectionMode,
    config: &rustscan_sfm::AdaptiveKeyframeSelectionConfig,
    frames: &[rustscan_sfm::SequenceFrame],
    mapper_config: &rustscan_sfm::MapperConfig,
    output: &Path,
    task: &mut rustscan_sfm::SfmTaskContext<'_>,
    run_adaptive: F,
) -> Result<ResolvedKeyframeSelection, RustSfmWorkerError>
where
    F: FnOnce(
        &[rustscan_sfm::SequenceFrame],
        &rustscan_sfm::AdaptiveKeyframeSelectionConfig,
        &rustscan_sfm::MapperConfig,
        &Path,
        &mut rustscan_sfm::SfmTaskContext<'_>,
    ) -> anyhow::Result<rustscan_sfm::AdaptiveKeyframeSelectionResult>,
{
    if frames.len() < 2 {
...
fn keyframe_stage_artifacts(
    ...
    reconstruction: &rustscan_sfm::KeyframeReconstructionResult,
...
fn hydrate_keyframe_result(
    ...
    frames: &[rustscan_sfm::SequenceFrame],
    output: &Path,
) -> Result<rustscan_sfm::KeyframeReconstructionResult, RustSfmWorkerError> {
    let stage = request
        .manifest
    ...
    Ok(rustscan_sfm::KeyframeReconstructionResult {
...
fn run_remaining_registration_with<F>(
    frames: &[rustscan_sfm::SequenceFrame],
    keyframes: &rustscan_sfm::KeyframeReconstructionResult,
    mapper_config: &rustscan_sfm::MapperConfig,
    registration_config: &rustscan_sfm::SequenceRegistrationConfig,
    output: &Path,
    task: &mut rustscan_sfm::SfmTaskContext<'_>,
    run_remaining: F,
) -> anyhow::Result<rustscan_sfm::SequenceRegistrationResult>
where
    F: FnOnce(
        &[rustscan_sfm::SequenceFrame],
        &[u32],
        &rustscan_sfm::KeyframeReconstructionResult,
        &rustscan_sfm::MapperConfig,
        &rustscan_sfm::SequenceRegistrationConfig,
        &Path,
        &mut rustscan_sfm::SfmTaskContext<'_>,
...
fn load_imported_sequence(request: &StageRequest) -> Result<ImportedSequence, RustSfmWorkerError> {
    ...
    for frame in imported {
        ...
        frames.push(rustscan_sfm::SequenceFrame {
            id: frame.id,
            image_path,
...
fn progress_sink(events: &WorkerEventSink) -> impl FnMut(rustscan_sfm::SfmTaskEvent) + '_ {
    move |event| {
        events.progress(
...
fn progress_operation_label(operation: rustscan_sfm::SfmTaskOperation) -> String {
    match operation {
        rustscan_sfm::SfmTaskOperation::MatchPairBatch => "Matching image pairs".to_owned(),
...
fn final_colmap_artifacts(
    ...
    output: &Path,
    frames: &[rustscan_sfm::SequenceFrame],
    registration: &rustscan_sfm::SequenceRegistrationResult,
) -> Result<Vec<PendingArtifact>, RustSfmWorkerError> {
...
mod tests {
    ...
    fn adaptive_selection_uses_only_rustsfm_selected_ids() {
        ...
        let control = rustscan_sfm::SfmTaskControl::new();
        let mut sink = |_| {};
        let mut task = rustscan_sfm::SfmTaskContext::new(&control, &mut sink);
    ...
    fn keyframe_stage_artifacts_persist_selection_and_reconstruction() {
        ...
        let selection = super::ResolvedKeyframeSelection {
            ...
            selection_config: Some(rustscan_sfm::AdaptiveKeyframeSelectionConfig::default()),
            evaluated_pairs: 2,
            diagnostics: adaptive_result(&[1, 3]).diagnostics,
        };
        let reconstruction = rustscan_sfm::KeyframeReconstructionResult {
            imported_frames: 3,
    ...
    fn remaining_registration_boundary_receives_hydrated_keyframes_and_all_frames() {
        ...
        let selected = sequence
            ...
            .map(|frame| frame.id)
            .collect::<Vec<_>>();
        let keyframes = rustscan_sfm::KeyframeReconstructionResult {
        ...
        let result = super::run_remaining_registration_with(
            ...
            |frames, ids, received_keyframes, _, _, received_output, _| {
                ...
                assert_eq!(received_output, output);
                Ok(rustscan_sfm::SequenceRegistrationResult {
    ...
    fn gpu_pnp_focal_backend_progress_distinguishes_cpu_fallback_reason() {
        let mut diagnostic = rustscan_sfm::FrameRegistrationDiagnostic::new(
            7,
            rustscan_sfm::FrameRegistrationStatus::Registered,
        );
        diagnostic.message = Some(
            "registered in Narrow round; gpu_pnp_focal_fallback=gpu dispatch failed".to_owned(),
        );
        let mut registration = rustscan_sfm::SequenceRegistrationResult {
            imported_frames: 2,
            registered_frames: 2,
            frame_ids: vec![0, 7],
            diagnostics: vec![
                rustscan_sfm::FrameRegistrationDiagnostic::new(
                    0,
                    rustscan_sfm::FrameRegistrationStatus::Keyframe,
                ),
                diagnostic,
            ],
    ...
    fn match_pair_batch_progress_uses_user_facing_label() {
        ...
        sink(rustscan_sfm::SfmTaskEvent {
            sequence: 11,
            elapsed_ms: 240,
            stage: rustscan_sfm::SfmTaskStage::FeatureMatching,
            operation: rustscan_sfm::SfmTaskOperation::MatchPairBatch,
            kind: rustscan_sfm::SfmTaskEventKind::Progress,
            completed: Some(32),
            total: Some(100),
    ...
    fn backend_completion_progress_is_emitted_only_for_successful_registration() {
        let registration = rustscan_sfm::SequenceRegistrationResult {
            imported_frames: 2,
            registered_frames: 2,
            frame_ids: vec![0, 1],
            diagnostics: Vec::new(),
        ...
        let incomplete = rustscan_sfm::SequenceRegistrationResult {
            registered_frames: 1,
            ..registration
        };
        let outcome = super::registration_outcome_with_backend_progress(
    ...
    fn adaptive_result(
        selected_frame_ids: &[u32],
    ) -> rustscan_sfm::AdaptiveKeyframeSelectionResult {
        rustscan_sfm::AdaptiveKeyframeSelectionResult {
            imported_frames: 2,
            usable_frames: 2,
            selected_frame_ids: selected_frame_ids.to_vec(),
            config: rustscan_sfm::AdaptiveKeyframeSelectionConfig::default(),
            evaluated_pairs: 1,
            diagnostics: vec![rustscan_sfm::AdaptiveKeyframePairDiagnostic {
                metrics: rustscan_sfm::AdaptiveKeyframePairMetrics {
                    anchor_frame_id: 1,
                ...
                decision: rustscan_sfm::AdaptiveKeyframeSelectionDecision::Boundary,
            }],
        }
    }
