...
use rustscan_gs::HostSplats;
use rustscan_viewer::pipeline::{
...
fn incomplete_pnp_coverage_needs_attention_and_never_starts_training() {
    ...
    assert!(
        std::iter::from_fn(|| coordinator.try_next_event()).any(|event| matches!(
            event,
            rustscan_viewer::pipeline::PipelineEvent::NeedsAttention(_)
        ))
    );
}
...
impl SfmWorker for CapturingSfmWorker {
    fn run(
        ...
        WorkerOutcome::Failed(rustscan_viewer::project::ProjectErrorRecord {
    ...
    fn run(
        ...
        if *failures_remaining > 0 {
            *failures_remaining -= 1;
            return WorkerOutcome::Failed(rustscan_viewer::project::ProjectErrorRecord {
                ...
                pair: None,
                retryable: true,
                suggested_actions: vec![rustscan_viewer::project::SuggestedAction::Retry],
            });
        }
        WorkerOutcome::Succeeded(vec![PendingArtifact::new(
            "result.json",
