...
use rustscan_gs::ColmapConfig;
use rustscan_sfm::colmap::export_colmap;
use rustscan_sfm::types::{CameraModel, Point3D, Reconstruction, TrackObservation};
use rustscan_slam::SE3;
use rustscan_viewer::loader::{checkpoint, load_colmap_training_dataset, mesh};
...
fn loads_rustsfm_export_with_one_shared_camera() {
    ...
    let reconstruction = Reconstruction {
        ...
        keypoints: vec![
            vec![rustscan_slam::KeyPoint::new(1.0, 1.0)],
            vec![rustscan_slam::KeyPoint::new(1.0, 1.0)],
        ],
        point_ids: vec![1],
        points: vec![Point3D {
...
#[test]
#[ignore = "requires checkpoint file at ../rustscan-slam/output/checkpoints/pipeline.json"]
fn test_load_checkpoint_from_output() {
    let checkpoint_path = Path::new("../rustscan-slam/output/checkpoints/pipeline.json");
...
#[test]
#[ignore = "requires test file at ../artifacts/inputs/middle/cube.obj"]
fn test_load_obj_cube() {
    let obj_path = Path::new("../artifacts/inputs/middle/cube.obj");
    if !obj_path.exists() {
...
#[test]
#[ignore = "requires test file at ../artifacts/inputs/large/FinalBaseMesh.obj"]
fn test_load_obj_finalbasemesh() {
    let obj_path = Path::new("../artifacts/inputs/large/FinalBaseMesh.obj");
    if !obj_path.exists() {
        eprintln!(
