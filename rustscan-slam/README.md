...
```bash
cargo build -p rustscan-slam
cargo test -p rustscan-slam --lib
```

For the library without the default video/CLI dependencies:

```bash
cargo check -p rustscan-slam --no-default-features --lib
cargo test -p rustscan-slam --no-default-features --lib
```
...
```bash
cargo run -p rustscan-slam --example load_tum_dataset --features image -- /path/to/tum/dataset
cargo run -p rustscan-slam --example e2e_slam_to_mesh --features image
```
