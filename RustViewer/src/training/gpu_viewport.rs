...
pub struct GpuViewportBridge {
    device: wgpu::Device,
    queue: wgpu::Queue,
    splat_cache: Option<GpuSplatCache>,
...
impl GpuViewportBridge {
    pub fn new(
        _context: rustgs::SharedWgpuContext,
        device: wgpu::Device,
        queue: wgpu::Queue,
    ...
    pub fn render_texture_id(
        ...
        self.ensure_splats_uploaded(splats.as_ref());
        let padded_count = self
            .splat_cache
            .as_ref()
            .ok_or_else(|| PreviewRenderError::RenderFailed("splat gpu cache missing".to_string()))?
            .padded_count;
        self.ensure_renderer(resolution, padded_count);
        let cache = self.splat_cache.as_ref().ok_or_else(|| {
            PreviewRenderError::RenderFailed("splat gpu cache missing".to_string())
        })?;
        let renderer = self.renderer.as_mut().ok_or_else(|| {
            PreviewRenderError::RenderFailed("gpu viewport renderer missing".to_string())
        })?;
        renderer.render(&self.device, &self.queue, cache, camera, resolution);
        if let Some(render_state) = render_state {
    ...
    fn ensure_splats_uploaded(&mut self, splats: &HostSplats) {
        ...
        self.splat_cache = Some(GpuSplatCache::new(&self.device, splats, key));
        self.renderer = None;
    }

    fn ensure_renderer(&mut self, resolution: PreviewResolution, padded_count: u32) {
        let needs_new = self.renderer.as_ref().is_none_or(|renderer| {
            renderer.resolution != resolution || renderer.padded_count != padded_count
        });
        if needs_new {
            self.renderer = Some(WgpuSplatRenderer::new(
                &self.device,
                resolution,
                padded_count,
            ));
...
impl GpuSplatCache {
    fn new(device: &wgpu::Device, splats: &HostSplats, key: SplatCacheKey) -> Self {
        let view = splats.as_view();
...
impl WgpuSplatRenderer {
    fn new(device: &wgpu::Device, resolution: PreviewResolution, padded_count: u32) -> Self {
