...
use clap::Parser;
use rustscan_sfm::colmap::{read_colmap_sparse_files, write_colmap_sparse_model};
use rustscan_sfm::feature_matching::MatchingPairStrategy;
use rustscan_sfm::generalized_pose::generalized_pose_capabilities;
use rustscan_sfm::sift::SiftMatchingOptions;
use rustscan_sfm::{
...
fn run_colmap_feature_extractor(args: ColmapFeatureExtractorArgs) -> Result<()> {
    ...
    if options.use_gpu {
        rustscan_sfm::gpu::validate_gpu_sift_options(&options)?;
    }
...
fn run_model_converter(args: ModelConverterArgs) -> Result<()> {
    ...
    let sparse = if let Some(format) = input_format {
        rustscan_sfm::colmap::read_colmap_sparse_files_with_format(&args.input_path, format)?
    } else {
    ...
    match cli.command {
        ...
        Commands::ExtractFeatures(args) => {
            ...
            let report = if args.taskflow {
                ...
                let executor = rustscan_sfm::CpuFeatureTaskflow::new(
                    runtime.clone(),
                ...
                let report = rustscan_sfm::extract_features_to_database_with_task(
                    &args.database,
                    &args.images,
                    &options,
                    &mut task,
