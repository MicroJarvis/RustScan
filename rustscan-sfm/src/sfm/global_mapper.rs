...
use crate::types::{CameraModel, ImageFrame, PairGeometry, Reconstruction, TrackObservation};
...
pub fn run_global_mapper(
    ...
    let rotation =
        estimate_global_rotations(num_views, &rotation_edges, &options.rotation_averaging)?;
...
pub fn pairwise_matches_from_pairs(
    pairs: &[PairGeometry],
) -> Vec<crate::track_establishment::PairwiseMatches> {
    pairs
        .iter()
...
fn run_global_reconstruction_component(
    ...
    let mapper = if used_joint_positioning {
...
fn run_per_image_triangulation_pass(
    ...
    let mut triangulator = IncrementalTriangulator::new(frames, pairs, reconstruction, tri_state);
...
fn track_has_min_triangulation_angle_filter(
    ...
    for i in 0..track.len() {
        for j in i + 1..track.len() {
            ...
            let angle_rad = crate::triangulation::calculate_triangulation_angle(&c1, &c2, &point3);
...
mod tests {
    ...
    use rustscan_slam::ColmapMt19937;
    use rustscan_slam::Match;
    ...
    fn synth_frame(id: usize, keypoints: Vec<rustscan_slam::KeyPoint>) -> ImageFrame {
    ...
    fn run_global_reconstruction_triangulates_shared_feature() {
        ...
        for i in 0..n {
            for j in (i + 1)..n {
                ...
                pairs.push(synth_pair_with_matches(i, j, &rotations, &centers, 0, 100));
            }
        }
        ...
        let (tracks, track_stats) = establish_tracks(&match_pairs, &options.tracks);
        let mut reconstruction = build_reconstruction_scaffold(&frames, camera, &mapper.poses);
        let triangulation_stats = triangulate_tracks(
            &tracks,
            &frames,
            &mut reconstruction,
            &options.triangulation,
    ...
    fn global_refinement_retriangulates_under_reconstructed_pairs() {
        ...
        frames[1].keypoints = vec![
            rustscan_slam::KeyPoint::new(330.0, 240.0),
            rustscan_slam::KeyPoint::new(340.0, 240.0),
        ];
    ...
    fn per_image_triangulation_creates_orphan_points_without_global_ba() {
        ...
        let mut frames = (0..3)
            ...
            .collect::<Vec<_>>();
        frames[1].keypoints[0] = rustscan_slam::KeyPoint::new(330.0, 240.0);
        frames[2].keypoints[0] = rustscan_slam::KeyPoint::new(340.0, 240.0);
