...
mod tests {
    ...
    use rustscan_slam::{Descriptors, Match};
    ...
    fn triangulate_image_ignores_two_view_tracks_by_default() {
        let mut frames = vec![frame(0), frame(1)];
        frames[1].keypoints = vec![rustscan_slam::KeyPoint::new(55.0, 50.0)];
    ...
    fn create_pair_track_builds_multiview_track_via_estimator() {
        ...
        frames[1].keypoints[0] = rustscan_slam::KeyPoint::new(62.5, 50.0);
        frames[2].keypoints[0] = rustscan_slam::KeyPoint::new(50.0, 62.5);
        let pairs = vec![
            pair(0, 1, &[(0, 0)]),
            pair(0, 2, &[(0, 0)]),
            pair(1, 2, &[(0, 0)]),
    ...
    fn complete_tracks_adds_transitive_observations() {
        let mut frames = vec![frame(0), frame(1), frame(2)];
        frames[1].keypoints[0] = rustscan_slam::KeyPoint::new(75.0, 50.0);
        frames[2].keypoints[0] = rustscan_slam::KeyPoint::new(100.0, 50.0);
    ...
    fn project_keypoint(pose: SE3, point: [f32; 3]) -> rustscan_slam::KeyPoint {
        ...
        rustscan_slam::KeyPoint::new(xy[0], xy[1])
    }
