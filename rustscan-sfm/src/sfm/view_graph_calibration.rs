...
fn epipoles_from_pair(
    ...
    if matches!(
        pair.two_view_config,
        COLMAP_TWO_VIEW_PLANAR | COLMAP_TWO_VIEW_PANORAMIC | COLMAP_TWO_VIEW_PLANAR_OR_PANORAMIC
    ) {
        return (None, None);
    }
...
pub fn filter_rotation_inconsistent_pairs(
    ...
    pairs
        .iter()
        .filter(|pair| rotation_edge_error_deg(pair, &rotation.global_rotations) <= max_error_deg)
        .cloned()
        .collect()
}
...
mod tests {
    ...
    fn test_frame(id: usize, x: f32, y: f32) -> ImageFrame {
        ImageFrame {
            ...
            width: 640,
            height: 480,
            keypoints: vec![rustscan_slam::KeyPoint::new(x, y)],
