...
fn build_poselib_bridge() {
    let poselib_root = resolve_poselib_root().unwrap_or_else(|| {
        panic!(
            "RustSFM poselib feature requires PoseLib v2.0.5 source. Set POSELIB_ROOT=/path/to/PoseLib-2.0.5, place it at rustscan-sfm/third_party/native/PoseLib, or at the workspace root third_party/native/PoseLib"
        )
    });
