...
Last re-evaluated: 2026-06-27, with `cargo test -p rustscan-sfm --lib`
...
and uses shared analytic reprojection Jacobians in the Ceres cost callback
...
PoseLib v2.0.5 is now vendored under `third_party/native/PoseLib` and the
...
(`rustscan-slam/src/colmap_rng.rs`), SIFT L1_ROOT/upright/scale-limit extraction,
...
- The PoseLib solver bridge now builds by default. PoseLib v2.0.5 is pinned as
  the `third_party/native/PoseLib` submodule and `build.rs` resolves it automatically,
...
1. Bundle adjustment / Ceres layer (`P5`, ~88%) — numerical-parity backbone is strong but not yet a 100% COLMAP boundary.
   - Done: Ceres robust loss family; Ceres-only BA dispatch; COLMAP 7D pose manifolds; rig/frame/sensor/intrinsics blocks; gauges; analytic Jacobians; structured Ceres termination and reduced-count fields; COLMAP CPU solver auto-selection; **sparse Schur covariance assembly**; **pose-prior BA** with f32-stable camera-center finite differences; **prior-position global BA gauge/normalization control-flow fixture**; **post-BA covariance**; **full solver-summary fields** (`linear_solver`, `preconditioner`, trust-region radius).
