...
fn main() -> Result<()> {
    ...
    for (chunk_idx, chunk) in inputs.chunks(BATCH).enumerate() {
        ...
        session.capacity() == BATCH,
        "batch-512 session should not grow"
    );
    ...
    let run = |session: &mut rustscan_sfm::gpu::FivePointSession<'_>,
     ...
     -> Result<String> {
        ...
        let right: Vec<[f32; 3]> = slice
            ...
            .flat_map(|i| i.right.iter().map(|r| [r.x as f32, r.y as f32, r.z as f32]))
            .collect();
        Ok(shared::model_signature(
            &session.solve_essential(&left, &right)?,
        ))
    };
    ...
    let time_session_pass = |session: &mut rustscan_sfm::gpu::FivePointSession<'_>| -> Result<f64> {
        let t0 = Instant::now();
        for chunk in &chunks {
            let left: Vec<[f32; 3]> = chunk
                .iter()
