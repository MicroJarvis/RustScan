...
let executor = rustscan_sfm::SfmTaskflow::new(runtime, 512 * 1024 * 1024)?;
let mut task = rustscan_sfm::SfmTaskContext::new(&control, &mut event_sink)
    .with_taskflow(executor.clone());
rustscan_sfm::run_reconstruction_with_task(&mapper_config, &mut task)?;
```
...
```rust,ignore
let runtime = std::sync::Arc::new(rustscan_taskflow::Runtime::new(config)?);
let features = rustscan_sfm::CpuFeatureTaskflow::new(runtime.clone(), 512 * 1024 * 1024, 32)?;
let ba = rustscan_sfm::CeresBaTaskflow::new(runtime.clone(), 2, 512 * 1024 * 1024)?;
let mut task = rustscan_sfm::SfmTaskContext::new(&control, &mut event_sink)
...
VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 \
  cargo run -p rustscan-sfm --release --example taskflow_ba --offline
```
...
```sh
cargo test -p rustscan-sfm --release --lib ba::taskflow --offline
cargo test -p rustscan-sfm --release --lib taskflow_ceres_and_sift --offline
cargo test -p rustscan-sfm --release --lib ba::ceres --offline
cargo test -p rustscan-sfm --release --test sequence_registration --offline
...
cargo run -p rustscan-sfm --release --bin rustsfm -- extract-features \
...
cargo run -p rustscan-sfm --release --example taskflow_features --offline
```
...
```rust,ignore
let runtime = std::sync::Arc::new(rustscan_taskflow::Runtime::new(config)?);
let features = rustscan_sfm::CpuFeatureTaskflow::new(
    runtime.clone(), 512 * 1024 * 1024, 32,
)?;
let mut task = rustscan_sfm::SfmTaskContext::new(&control, &mut event_sink)
    .with_cpu_feature_taskflow(&features);
rustscan_sfm::extract_features_to_database_with_task(&database, &images, &options, &mut task)?;
```
...
```sh
cargo test -p rustscan-sfm --release --lib feature_extraction::tests --offline
cargo test -p rustscan-sfm --release --no-default-features --lib feature_extraction::tests::taskflow --offline
cargo test -p rustscan-sfm --release --bin rustsfm cli::tests --offline
cargo test -p rustscan-sfm --release --test task_control --offline
cargo test -p rustscan-sfm --release --test taskflow_features --offline
```
...
cargo test -p rustscan-sfm --release --lib
```
...
`artifacts/inputs/flowers2_colmap` 24-image sparse fixture. That runtime tree is not
...
`artifacts/inputs/fixtures/flowers2_colmap_ref_text_20260630` (SHA-256 verified).
Provision it, then run the ignored suite explicitly:

```bash
./scripts/provision_flowers2_colmap_fixture.sh
cargo test -p rustscan-sfm --lib -- --ignored
```
