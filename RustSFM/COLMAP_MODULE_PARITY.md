...
passing 350 tests (Ceres BA enabled by default via `ceres-ba` feature),
`cargo test -p rustsfm --features poselib --lib` passing 353 tests, and
`cargo test -p rustsfm --no-default-features --lib` passing 337 tests (native
...
1. Bundle adjustment / Ceres layer (`P5`, ~80%) — largest numerical-parity
   ...
   - Done: Ceres robust loss family (Trivial/Huber/SoftL1/Cauchy) with COLMAP's
     ...
     and sensor pose residual blocks, explicit Ceres `TrivialLoss`, structured
     Ceres termination fields, and COLMAP CPU solver auto-selection thresholds
