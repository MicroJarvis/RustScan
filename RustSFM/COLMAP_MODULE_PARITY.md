...
passing 329 tests (Ceres BA enabled by default via `ceres-ba` feature),
...
1. Bundle adjustment / Ceres layer (`P5`, ~78%) — largest numerical-parity
   ...
   - Done: Ceres robust loss family (Trivial/Huber/SoftL1/Cauchy) with COLMAP's
     ...
     native projection code, and exact COLMAP ambient Jacobians for ordinary
     image pose residuals.
   - Next: exact analytic ambient Jacobians for frame/sensor composite Ceres
     pose manifolds,
     jacobian-scaled LM damping / Ceres trust-region update, true sparse Schur
     storage, covariance, normalization, and exact Ceres solver summaries.
