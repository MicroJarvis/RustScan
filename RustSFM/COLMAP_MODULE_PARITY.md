...
Last re-evaluated: 2026-06-21, with `cargo test -p rustsfm --lib`
passing 353 tests (Ceres BA enabled by default via `ceres-ba` feature),
`cargo test -p rustsfm --features poselib --lib` passing 358 tests, and
`cargo test -p rustsfm --no-default-features --lib` passing 338 tests (native
BA only). Ceres BA (`ceres_problem.rs`) now supports rig/frame/sensor pose
blocks, intrinsics refinement, gauge policies, and COLMAP-style CPU
linear-solver auto-selection, and reuses native analytic reprojection Jacobians
in the Ceres cost callback (numeric fallback retained).
...
| P5 | Bundle adjustment / `optim` Ceres layer | `ba/` | **~80%** ↑ | No | Parameter block scheduling, analytic Jacobians (native + **Ceres path now reuses native analytic projection/frame/sensor/camera Jacobians** with numeric fallback), gauges, structured Ceres iteration-summary/termination/reduced-count fields, convergence knobs including checked Ceres integer option forwarding and `max_linear_solver_iterations`, COLMAP-style Ceres CPU threading gate, COLMAP CPU linear-solver auto-selection (`DENSE_SCHUR`/`SPARSE_SCHUR` when Ceres exposes sparse support/`ITERATIVE_SCHUR` + `SCHUR_JACOBI` thresholds), active Ceres BA 7D quaternion+translation pose manifold blocks, COLMAP raw Eigen-quaternion image/frame/sensor pose residuals with exact 2x7 ambient Jacobians, and Ceres-equivalent loss family including explicit Ceres `TrivialLoss` plus COLMAP-style robust scale validation. The `ba/` module splits into `mod.rs`, `shared.rs`, `native.rs`, `ceres.rs`, and `ceres_problem.rs` (full Ceres problem: image/frame/sensor pose blocks, intrinsics, gauges, fixed poses). `ceres-ba` is **enabled by default**; no fallback to native. Remaining gaps: covariance, true sparse Schur storage, full backend solver-summary parity, and remaining trust-region/LM damping details. |
| P5 | Local/global BA orchestration | `mapper.rs`, `ba.rs`, `observation_manager.rs` | 55% | No | Local/global BA scheduling, gauge handling, frame-aware options, post-registration constant-camera decisions, COLMAP incremental local/global Ceres loss and iteration defaults, local iterative refinement count/change defaults, robust-to-trivial local refinement loss handoff, COLMAP small-reconstruction global BA convergence tightening, incremental global-BA reconstruction normalization, COLMAP `FindRedundantPoints3D` selection primitive, and post-BA filtering are partial. Exact image/point selection, local filtering scope, pose priors, covariance, redundant-point global BA pass wiring, final-all handoff plumbing, and Ceres behavior remain incomplete. |
...
1. Bundle adjustment / Ceres layer (`P5`, ~80%) — largest numerical-parity
   ...
   - Done: Ceres robust loss family (Trivial/Huber/SoftL1/Cauchy) with COLMAP's
     ...
     native projection code, exact COLMAP ambient Jacobians for image, frame,
     and sensor pose residual blocks, explicit Ceres `TrivialLoss`, robust loss
     scale validation, checked Ceres integer option forwarding, structured
     Ceres termination and reduced-count fields, and COLMAP CPU solver
     auto-selection thresholds plus sparse-backend gating through
     `ITERATIVE_SCHUR` + `SCHUR_JACOBI`.
   - Next: jacobian-scaled LM damping / Ceres trust-region update, true sparse
     Schur storage, covariance, pose-prior BA integration, and exact Ceres
     solver summaries.
