...
pub fn compare_colmap_stages(
    ...
    let resolved_reference_database = if needs_database {
        Some(resolve_compare_database_path(reference_sparse, reference_database)?)
    } else {
        reference_database.map(Path::to_path_buf)
    };
    let resolved_candidate_database = if needs_database {
        Some(resolve_compare_database_path(candidate_sparse, candidate_database)?)
    } else {
...
fn raw_match_counts_by_name(db: &ColmapDatabase) -> Result<HashMap<PairNameKey, usize>> {
    ...
    for (pair_id, matches) in db.read_all_matches()? {
        let (image_id1, image_id2) = pair_id_to_image_pair(pair_id)
...
pub fn compare_feature_counts(
    ...
    let reference_names = reference.keys().cloned().collect::<std::collections::BTreeSet<_>>();
    let candidate_names = candidate.keys().cloned().collect::<std::collections::BTreeSet<_>>();
...
fn compare_pair_count_maps(
    ...
    let reference_keys = reference.keys().cloned().collect::<std::collections::BTreeSet<_>>();
    let candidate_keys = candidate.keys().cloned().collect::<std::collections::BTreeSet<_>>();
    ...
    for key in &common_keys {
        ...
        *config_confusion
        ...
        let overlap = if reference_set.is_empty() {
            ...
            reference_set.intersection(&candidate_set).count() as f64
    ...
    config_mismatches.sort_by(|left, right| {
        right
            ...
            .cmp(
                &left
                    .reference_inliers
                    .abs_diff(left.candidate_inliers),
            )
    });
...
mod tests {
    ...
    fn compare_colmap_stages_runs_sparse_only_stages() {
        ...
        let report = compare_colmap_stages(
            ...
            None,
            None,
            &[CompareStage::Registration, CompareStage::Tracks, CompareStage::Ba],
        )
        .unwrap();
        let registration = report.registration.expect("registration");
        assert_eq!(registration.reference_registered, registration.candidate_registered);
