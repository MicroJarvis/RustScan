...
target/release/examples/five_point_gpu_replay \
  ...
  --pairs 2 --trials 16 --rounds 3 --output output/five_point_gpu_debug.json
# Final measured workload (both batch sizes are run by the example).
target/release/examples/five_point_gpu_replay \
  --database /Users/tfjiang/Projects/RustScan/output/flowers2_960_settlement_20260913/matching.db \
  --pairs 12 --trials 512 --rounds 3 --output output/five_point_gpu_replay_12x512_final.json
```
...
Final artifact: `output/five_point_gpu_replay_12x512_final.json` (22,494,838 bytes).
