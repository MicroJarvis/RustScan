...
fn build_ray_residual_cost(binding: RayResidualBinding) -> CostFunctionType<'static> {
    Box::new(
        move |parameters: &[&[f64]], residuals: &mut [f64], jacobians| {
            ...
            let Some((residual, _)) =
                ray_direction_residual_and_jacobian(center, point, binding.ray, binding.sqrt_weight)
            else {
                residuals.fill(0.0);
            ...
            if let Some(jacobians) = jacobians {
                fill_numeric_ray_jacobians(center, point, binding.ray, binding.sqrt_weight, jacobians);
            }
            true
        },
    )
        ...
        for point in &gt_points {
            ...
            for view in 0..n {
                ...
                if observations.is_empty() {
                    ...
                    frames[view].keypoints.push(rustslam::KeyPoint::new(kp.0, kp.1));
