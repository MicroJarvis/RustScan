...
pub fn triangulate_tracks(
    ...
    for track in tracks {
        ...
        let Some(xyz) = triangulate_track_observations(
            &observations,
            frames,
            reconstruction,
        ) else {
        ...
        if options.require_cheirality
            && !track_is_cheiral(xyz, &observations, reconstruction)
        {
        ...
        let Some(error) =
            mean_track_reprojection_error(xyz, &observations, frames, reconstruction)
        else {
...
mod tests {
    ...
    fn triangulates_synthetic_track_with_known_point() {
        ...
        let track = Track {
            observations: (0..n)
                .map(|view| FeatureNode::new(view, 0))
                .collect(),
        };
