...
use crate::database::{
    ColmapDatabase, ColmapDescriptors, ColmapKeypoint, COLMAP_FEATURE_SIFT,
};
...
pub fn extract_features_to_database(
    ...
    let total_keypoints = reports.iter().map(|image| image.num_keypoints).sum::<usize>();
...
fn sift_backend_name() -> &'static str {
    if cfg!(all(feature = "vlfeat-sift", not(feature = "lowe-sift-backend"))) {
        "vlfeat"
    } else {
        "lowe-sift"
    }
