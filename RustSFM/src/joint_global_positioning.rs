...
pub(crate) struct RayObservation {
    camera: usize,
    feature: usize,
    track: usize,
    ray_world: Vec3,
...
pub fn estimate_joint_global_positions(
    ...
    let mut centers = initialize_centers(
        num_views,
        global_rotations,
        pairs,
        &observations,
    ...
    let mut points =
        initialize_points_from_poses(num_tracks, &observations, &centers, global_rotations, frames, camera);
    ...
    let (centers, points, num_iterations, mean_residual) = match options.solver {
        ...
        #[cfg(not(feature = "ceres-ba"))]
        JointGlobalPositioningSolver::CeresLevenbergMarquardt => solve_joint_global_positioning_alternating(
            &observations,
            num_views,
            num_tracks,
            centers,
...
pub(crate) fn solve_joint_global_positioning_alternating(
    ...
    for _ in 0..options.max_num_iterations {
        ...
        let weights =
            robust_weights(observations, &centers, &points, &depths, options.huber_threshold);
...
mod tests {
    ...
    fn project(camera: CameraModel, rotation: Quat, center: Vec3, point: Vec3) -> Option<(f32, f32)> {
        ...
        let xy = camera
            .img_from_cam(cam[0] as f64, cam[1] as f64, cam[2] as f64)?;
    ...
    fn synth_pair(i: usize, j: usize, rotations: &[Quat], centers: &[Vec3], inliers: usize) -> PairGeometry {
        ...
        for point in &gt_points {
            ...
            for view in 0..n {
                ...
                if observations.is_empty() {
                    ...
                    frames[view]
                        .keypoints
                        .push(rustslam::KeyPoint::new(x, y));
                    frames[view].colors.push([128, 128, 128]);
                    observations.push(FeatureNode::new(view, feature_idx));
                }
            }
