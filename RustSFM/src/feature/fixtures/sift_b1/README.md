...
no images, extraction backend, ignored `output/`, Python, network, or regeneration
...
Source: `RustSFM/output/sift_distance_kernel/corpus/{left.bin,right.bin,metadata.json}`,
frozen by `RustSFM/examples/sift_search_bench.rs extract` using default SIFT
extraction options except `max_num_features = 8192`. The recorded images are
`test_data/flowers/images/frame_0001.jpg` and `frame_0002.jpg` (flowers, not flowers2).
Original counts: **5,865 left / 5,548 right**.

Selection from historical `RustSFM/output/sift_distance_kernel/after.json`:
...
```sh
python3 RustSFM/src/feature/fixtures/sift_b1/reproduce.py RustSFM/output/sift_distance_kernel/corpus
```
...
```sh
VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 cargo test -p rustsfm --release --lib --offline b1_ -- --test-threads=1
VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 cargo test -p rustsfm --release --lib --offline --no-default-features sift_index:: -- --test-threads=1
VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 cargo check -p rustsfm --release --lib --offline --no-default-features
```
