...
use rustsfm::{SfmTaskContext, SfmTaskControl, SfmTaskflow, SiftExtractionOptions};
    ...
    let results = std::thread::scope(|scope| {
        ...
        for (id, path) in databases.iter().enumerate() {
            ...
            handles.push(scope.spawn(move || -> Result<Value> {
                ...
                let result = rustsfm::extract_features_to_database_with_task(path, input, options, &mut task);
