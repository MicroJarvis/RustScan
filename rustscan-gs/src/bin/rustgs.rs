...
struct TrainArgs {
    /// Path to a COLMAP dataset directory with sparse reconstruction points
    #[arg(short, long)]
    input: PathBuf,
    ...
    #[arg(short, long, default_value = "artifacts/runs/scene.ply")]
    output: PathBuf,
    ...
    /// Training preset name loaded from the dataset training config file
    #[arg(long)]
    ...
    #[arg(long, default_value_t = rustscan_gs::DEFAULT_RASTER_COV_BLUR)]
    raster_cov_blur: f32,
    ...
    #[arg(long, default_value_t = rustscan_gs::LiteGsTrainingProfile::Baseline)]
    litegs_profile: rustscan_gs::LiteGsTrainingProfile,

    /// LiteGS tile size, e.g. 8x16 or 8,16
    #[arg(long, default_value_t = rustscan_gs::LiteGsTileSize::new(8, 16))]
    litegs_tile_size: rustscan_gs::LiteGsTileSize,
    ...
    #[arg(long, default_value_t = rustscan_gs::LiteGsSplitScoreMode::Baseline)]
    litegs_split_score: rustscan_gs::LiteGsSplitScoreMode,
    ...
    #[arg(long, default_value_t = rustscan_gs::LiteGsOpacityResetMode::Decay)]
    litegs_opacity_reset_mode: rustscan_gs::LiteGsOpacityResetMode,

    /// LiteGS prune mode
    #[arg(long, default_value_t = rustscan_gs::LiteGsPruneMode::Weight)]
    litegs_prune_mode: rustscan_gs::LiteGsPruneMode,
...
enum Commands {
    /// Train a 3DGS scene from a COLMAP dataset directory
    Train(TrainArgs),
...
mod tests {
    ...
    fn training_defaults_to_lossless_ply_artifact() {
        ...
        assert_eq!(args.output, PathBuf::from("artifacts/runs/scene.ply"));
    ...
    fn training_options_preserve_defaults_without_checkpoint_flags() {
        ...
        let dataset = rustscan_gs::TrainingDataset::new(rustscan_gs::Intrinsics::new(
            1.0, 1.0, 0.5, 0.5, 1, 1,
        ));

        let options = train_command::training_options(
            &args,
            &dataset,
    ...
    fn training_options_load_resume_checkpoint_before_training() {
        ...
        let error = train_command::training_options(
            &args,
            &dataset,
            &rustscan_gs::TrainingConfig::default(),
        )
    ...
    fn direct_sparse_training_identity_ignores_non_model_artifacts() {
        ...
        .unwrap();
        let mut changed_target = rustscan_gs::TrainingConfig {
            iterations: 1_200,
            ..Default::default()
        };
        let added =
...
fn run_prune_scene_command(args: PruneSceneArgs) -> anyhow::Result<()> {
    let _ = env_logger::Builder::new()
        ...
        .try_init();
    let (splats, mut metadata) = rustscan_gs::load_splats(&args.input)?;
    ...
    rustscan_gs::save_splats(&args.output, &pruned, &metadata)?;
...
fn prune_splats(
    ...
    args: &PruneSceneArgs,
) -> anyhow::Result<(rustscan_gs::HostSplats, PruneSceneSummary)> {
    ...
    let pruned = rustscan_gs::HostSplats::from_components(
        kept_positions,
        kept_log_scales,
        kept_rotations,
        kept_opacities,
...
#[cfg(feature = "gpu")]
fn scene_center_and_extent(view: &rustscan_gs::SplatView<'_>) -> ([f32; 3], f32) {
...
fn run_compare_optimization_reports_command(
    ...
    let baseline = rustscan_gs::load_optimization_report(&args.baseline).with_context(|| {
        format!(
            "failed to load baseline optimization report {}",
            args.baseline.display()
        )
    })?;
    let candidate = rustscan_gs::load_optimization_report(&args.candidate).with_context(|| {
        format!(
            "failed to load candidate optimization report {}",
            args.candidate.display()
        )
    })?;
    let result = rustscan_gs::compare_optimization_reports(&baseline, &candidate);
    if args.json {
        #[derive(Serialize)]
        struct CompareJson<'a> {
            decision: &'a str,
            reasons: Vec<&'a str>,
            deltas: &'a [rustscan_gs::OptimizationMetricDelta],
        }
        let (decision, reasons) = match &result.decision {
            rustscan_gs::OptimizationCompareDecision::Compatible => ("compatible", Vec::new()),
            rustscan_gs::OptimizationCompareDecision::Rejected { reasons } => {
        ...
        match &result.decision {
            rustscan_gs::OptimizationCompareDecision::Compatible => {
                println!("compatible");
            }
            rustscan_gs::OptimizationCompareDecision::Rejected { reasons } => {
                println!("rejected");
    ...
    if matches!(
        result.decision,
        rustscan_gs::OptimizationCompareDecision::Rejected { .. }
    ) {
...
fn run_render_command(args: RenderArgs) -> anyhow::Result<()> {
    ...
    let (splats, metadata) = rustscan_gs::load_splats(&args.input)?;
    ...
    let mut renderer = rustscan_gs::SplatEvaluationRenderer::new(
        ...
        rustscan_gs::EvaluationDevice::Gpu,
...
struct RenderCameraFile {
    intrinsics: rustscan_gs::Intrinsics,
    pose: rustscan_gs::SE3,
    #[serde(default)]
    pose_is_world_to_camera: bool,
}

#[cfg(feature = "gpu")]
fn load_render_camera(path: &std::path::Path) -> anyhow::Result<rustscan_gs::GaussianCamera> {
    ...
    Ok(rustscan_gs::GaussianCamera::new(
        camera.intrinsics,
        extrinsics,
    ))
}
