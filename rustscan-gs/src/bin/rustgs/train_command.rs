...
impl<S> CheckpointPersistence<S> {
    ...
    fn persist<T, E>(
        &mut self,
        reason: rustscan_gs::TrainingCheckpointReason,
        iteration: usize,
        checkpoint: &T,
    ...
    {
        let path = if reason == rustscan_gs::TrainingCheckpointReason::Periodic {
            self.directory
                .join(format!("iteration-{iteration:06}.rgscp"))
        } else {
            let prefix = match reason {
                rustscan_gs::TrainingCheckpointReason::Pause => "pause",
                rustscan_gs::TrainingCheckpointReason::Shutdown => "shutdown",
                rustscan_gs::TrainingCheckpointReason::Periodic => unreachable!(),
            };
...
pub(super) fn training_options(
    ...
    let reconstruction = rustscan_gs::fingerprint_colmap_sparse_model(&args.input)?;
    let identity =
    ...
    if let Some(resume_path) = &args.resume {
        ...
        .with_context(|| format!("failed to load resume checkpoint {}", resume_path.display()))?;
    ...
    if let (Some(directory), Some(every)) = (&args.checkpoint_dir, checkpoint_every) {
        let mut persistence = CheckpointPersistence::new(
            directory.clone(),
            rustscan_gs::save_training_checkpoint
                as fn(
                    &Path,
                    &rustscan_gs::TrainingCheckpoint,
                ) -> Result<(), rustscan_gs::TrainingError>,
        );
        options = options
            .with_checkpoint_policy(rustscan_gs::TrainingCheckpointPolicy { every: Some(every) })
...
pub(super) fn run_train_command(args: TrainArgs, sources: TrainArgSources) -> anyhow::Result<()> {
    ...
    let metadata = rustscan_gs::SplatMetadata {
...
pub(super) struct TrainConfigOverrides {
    ...
    litegs_profile: Option<rustscan_gs::LiteGsTrainingProfile>,
    #[serde(default, deserialize_with = "deserialize_optional_litegs_tile_size")]
    litegs_tile_size: Option<rustscan_gs::LiteGsTileSize>,
    ...
    #[serde(default, deserialize_with = "deserialize_optional_from_str")]
    litegs_split_score: Option<rustscan_gs::LiteGsSplitScoreMode>,
    ...
    #[serde(default, deserialize_with = "deserialize_optional_from_str")]
    litegs_opacity_reset_mode: Option<rustscan_gs::LiteGsOpacityResetMode>,
    #[serde(default, deserialize_with = "deserialize_optional_from_str")]
    litegs_prune_mode: Option<rustscan_gs::LiteGsPruneMode>,
...
fn deserialize_optional_litegs_tile_size<'de, D>(
    deserializer: D,
) -> Result<Option<rustscan_gs::LiteGsTileSize>, D::Error>
where
...
impl LiteGsTileSizeConfig {
    fn into_tile_size(self) -> Result<rustscan_gs::LiteGsTileSize, String> {
        match self {
            Self::String(value) => rustscan_gs::LiteGsTileSize::from_str(&value),
            Self::Object { width, height } => {
                if width == 0 || height == 0 {
                    Err("litegs_tile_size width and height must both be > 0".to_string())
                } else {
                    Ok(rustscan_gs::LiteGsTileSize::new(width, height))
                }
            }
        }
    }
...
pub(super) fn load_training_dataset_for_training(
    ...
    rustscan_types::TrainingDataset,
    rustscan_gs::TrainingInputKind,
)> {
...
) -> Option<rustscan_gs::FinalTrainingMetrics> {
    training_telemetry.map(|telemetry| rustscan_gs::FinalTrainingMetrics {
...
fn crop_frame_indices(
    dataset: &rustscan_types::TrainingDataset,
    summary: &rustscan_gs::SplatEvaluationSummary,
...
fn export_evaluation_crops(
    ...
    device: &rustscan_gs::EvaluationDevice,
    summary: &rustscan_gs::SplatEvaluationSummary,
    ...
    let runtime_splats =
        rustscan_gs::runtime_from_splats(splats, device).map_err(anyhow::Error::from)?;
    let mut renderer = rustscan_gs::SplatEvaluationRenderer::new(
        summary.render_width,
        summary.render_height,
        *device,
    ...
    for idx in frame_indices {
        ...
        let (target, rendered) = rustscan_gs::render_evaluation_frame(
            &selected,
            pose,
            summary.render_width,
            summary.render_height,
...
pub(super) fn build_training_config(
    args: &TrainArgs,
) -> anyhow::Result<rustscan_gs::TrainingConfig> {
    ...
    let config = rustscan_gs::TrainingConfig {
        ...
        initialization: rustscan_gs::TrainingInitializationConfig {
            ...
            ..rustscan_gs::TrainingInitializationConfig::default()
        },
        data: rustscan_gs::TrainingDataConfig {
            frame_shuffle_seed: args.frame_shuffle_seed,
            ..rustscan_gs::TrainingDataConfig::default()
        },
        ...
        optimizer: rustscan_gs::TrainingOptimizerConfig {
        ...
        loss: rustscan_gs::TrainingLossConfig {
            ...
            loss_dynamic_mask_gradient: args
                .loss_dynamic_mask_gradient
                .parse::<rustscan_gs::DynamicMaskGradient>()
                .map_err(anyhow::Error::msg)?,
        },
        litegs: rustscan_gs::LiteGsConfig {
            rendering: rustscan_gs::LiteGsRenderingConfig {
                sh_degree: args.litegs_sh_degree,
                tile_size: args.litegs_tile_size,
            },
            features: rustscan_gs::LiteGsFeatureConfig {
            ...
            topology: rustscan_gs::LiteGsTopologyConfig {
            ...
            growth: rustscan_gs::LiteGsGrowthConfig {
            ...
            refine: rustscan_gs::LiteGsRefineConfig {
                opacity_decay: args.litegs_opacity_decay,
                scale_decay: args.litegs_scale_decay,
            },
            pruning: rustscan_gs::LiteGsPruningConfig {
            ...
            camera: rustscan_gs::LiteGsCameraConfig {
...
fn litegs_profile_overrides(args: &TrainArgs) -> (rustscan_gs::LiteGsSplitScoreMode, f32, f32) {
    match args.litegs_profile {
        rustscan_gs::LiteGsTrainingProfile::Baseline => (
            args.litegs_split_score,
            args.litegs_split_grad_threshold,
            args.litegs_depth_scale_gamma,
        ),
        rustscan_gs::LiteGsTrainingProfile::AbsSplit => (
            rustscan_gs::LiteGsSplitScoreMode::Abs,
            0.00001,
            args.litegs_depth_scale_gamma,
        ),
        rustscan_gs::LiteGsTrainingProfile::AbsPixel => (
            rustscan_gs::LiteGsSplitScoreMode::AbsPixel,
            0.00001,
            args.litegs_depth_scale_gamma,
        ),
        rustscan_gs::LiteGsTrainingProfile::AbsPixelDepth => (
            rustscan_gs::LiteGsSplitScoreMode::AbsPixelDepth,
            0.00001,
            args.litegs_depth_scale_gamma,
        ),
    }
...
fn log_litegs_training_config(config: &rustscan_gs::TrainingConfig) {
    log::info!(
...
fn ensure_sparse_initialization_points(
    dataset: &rustscan_types::TrainingDataset,
    source: rustscan_gs::TrainingInputKind,
    input: &Path,
) -> anyhow::Result<()> {
...
fn resolve_optimization_report_path(args: &TrainArgs) -> Option<PathBuf> {
    ...
    if args.eval_json {
        return Some(rustscan_gs::default_optimization_report_path(&args.output));
    }
    None
}
...
fn optimization_evaluation_metrics(
    evaluation: &rustscan_gs::SplatEvaluationResult,
) -> rustscan_gs::OptimizationEvaluationMetrics {
...
fn maybe_write_optimization_report(
    ...
    let report = rustscan_gs::build_optimization_report(
        rustscan_gs::OptimizationEnvironment {
            ...
            adapter_name: gpu_profiler.and_then(|p| p.adapter.clone()),
            backend: gpu_profiler
                .map(|p| p.backend.clone())
                .or_else(|| Some("wgpu".to_string())),
            driver: gpu_profiler.and_then(|p| p.driver.clone()),
            timestamp_query_available: gpu_profiler.map(|p| p.supported),
            adapter_unavailable_reason: gpu_profiler
                .and_then(|p| p.adapter_unavailable_reason.clone()),
            driver_unavailable_reason: gpu_profiler
                .and_then(|p| p.driver_unavailable_reason.clone()),
        },
        rustscan_gs::OptimizationCommand {
            ...
            loss_config_fingerprint: rustscan_gs::canonical_config_fingerprint(&config.loss).ok(),
            topology_config_fingerprint: rustscan_gs::canonical_config_fingerprint(
                &config.litegs.topology,
            )
            .ok(),
            sh_schedule_fingerprint: rustscan_gs::sh_schedule_fingerprint(
        ...
        rustscan_gs::OptimizationTrainMetrics {
            ...
            gpu_completion_seconds: gpu_profiler.and_then(|p| {
                p.gpu_step_p50_ms
                    .map(|ms| ms / 1000.0 * completed_iterations as f64)
            }),
            gpu_step_p50_ms: gpu_profiler.and_then(|p| p.gpu_step_p50_ms),
            gpu_step_p95_ms: gpu_profiler.and_then(|p| p.gpu_step_p95_ms),
            gpu_step_sample_count: gpu_profiler.map(|p| p.sample_count),
            gpu_profiler_unsupported_reason: gpu_profiler
                .and_then(|p| p.unsupported_reason.clone()),
            ...
            status_readbacks_loss_cadence: telemetry.and_then(|t| t.status_readbacks_loss_cadence),
            status_readbacks_topology: telemetry.and_then(|t| t.status_readbacks_topology),
            status_readbacks_checkpoint: telemetry.and_then(|t| t.status_readbacks_checkpoint),
        ...
        rustscan_gs::OptimizationTopologyMetrics {
        ...
        rustscan_gs::OptimizationMemoryMetrics {
            ...
            peak_device_bytes: gpu_profiler.and_then(|p| p.runtime_peak_device_bytes),
            peak_device_bytes_reason: gpu_profiler
                .and_then(|p| p.runtime_peak_device_bytes_reason.clone()),
            estimated_buffer_bytes,
            workspace_current_bytes: gpu_profiler.map(|p| p.workspace_current_bytes),
            workspace_peak_bytes: gpu_profiler.map(|p| p.workspace_peak_bytes),
            workspace_growth_count: gpu_profiler.map(|p| p.workspace_growth_count),
            fresh_step_allocations: gpu_profiler.map(|p| p.fresh_step_allocations),
        },
        evaluation.map(optimization_evaluation_metrics),
    );

    rustscan_gs::write_optimization_report(&report_path, &report).with_context(|| {
        format!(
...
pub(super) fn maybe_write_litegs_parity_report(
    ...
    evaluation_summary: Option<&rustscan_gs::SplatEvaluationSummary>,
) -> anyhow::Result<()> {
...
pub(super) fn maybe_write_litegs_parity_report_with_manifest_dir(
    ...
    let report_path = rustscan_gs::default_parity_report_path(output);
    let fixture_id = rustscan_gs::parity_fixture_id_for_input_path(input);
    let mut report = rustscan_gs::ParityHarnessReport::new(fixture_id, &config.litegs);
    ...
    let (roundtrip_splats, roundtrip_metadata) = rustscan_gs::load_splats(output)?;
    report.metrics.export_roundtrip_ok = matches!(
        rustscan_gs::splat_artifact_fidelity(output)?,
        rustscan_gs::SplatArtifactFidelity::Lossless
    ) && rustscan_gs::verify_lossless_roundtrip(
        splats,
        metadata,
        &roundtrip_splats,
        &roundtrip_metadata,
    ...
    if matches!(
        rustscan_gs::splat_artifact_fidelity(output)?,
        rustscan_gs::SplatArtifactFidelity::LossyLegacy
    ) {
        report.notes.push(
    ...
    {
        match rustscan_gs::ParityHarnessReport::load_json(&reference_report_path) {
            Ok(reference_report) => {
                ...
                report.reference_comparison = rustscan_gs::compare_loss_curve_samples(
    ...
    } else if report.fixture_id == rustscan_gs::DEFAULT_CONVERGENCE_FIXTURE_ID {
        report.notes.push(
...
fn resolve_parity_reference_report_path_from_manifest_dir(
    ...
    manifest_dir
        .ancestors()
        .find_map(|path| rustscan_gs::resolve_litegs_parity_reference_report_path(fixture_id, path))
}

#[cfg(feature = "gpu")]
fn inferred_initialization_gaussian_count(
    dataset: &rustscan_types::TrainingDataset,
    _config: &rustscan_gs::TrainingConfig,
) -> Option<usize> {
...
#[cfg(feature = "gpu")]
fn splats_have_non_finite(splats: &rustscan_gs::HostSplats) -> bool {
...
mod checkpoint_persistence_tests {
    ...
    fn checkpoint_persistence_retains_only_the_newest_three_periodic_files() {
        ...
        for iteration in 1..=4 {
            persistence
                .persist(
                    rustscan_gs::TrainingCheckpointReason::Periodic,
                    iteration,
                    &iteration,
                )
                .unwrap();
    ...
    fn checkpoint_persistence_never_prunes_non_periodic_reasons() {
        ...
        let pause_path = persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Pause, 1, &1)
            .unwrap()
            .path;
        let shutdown_path = persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Shutdown, 1, &1)
            .unwrap()
            .path;
    ...
    fn checkpoint_persistence_restores_periodic_retention_after_restart() {
        ...
        let mut persistence =
            CheckpointPersistence::new(temp.path().to_path_buf(), write_test_checkpoint);
        persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Periodic, 5, &5)
            .unwrap();
    ...
    fn checkpoint_persistence_ignores_noncanonical_periodic_names() {
        ...
        let mut persistence =
            CheckpointPersistence::new(temp.path().to_path_buf(), write_test_checkpoint);
        persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Periodic, 4, &4)
            .unwrap();
    ...
    fn duplicate_non_periodic_checkpoints_use_unique_paths_across_restarts() {
        let temp = tempfile::tempdir().unwrap();
        for reason in [
            rustscan_gs::TrainingCheckpointReason::Pause,
            rustscan_gs::TrainingCheckpointReason::Shutdown,
        ] {
            let first_path = {
                let mut persistence =
                    CheckpointPersistence::new(temp.path().to_path_buf(), write_test_checkpoint);
    ...
    fn concurrent_non_periodic_path_selection_does_not_collide() {
        ...
        let spawn_writer = |checkpoint: usize| {
            ...
            thread::spawn(move || {
                ...
                persistence
                    .persist(rustscan_gs::TrainingCheckpointReason::Pause, 1, &checkpoint)
                    .unwrap()
                    .path
            })
        };
    ...
    fn duplicate_periodic_checkpoint_replaces_the_canonical_path() {
        ...
        let first_path = persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Periodic, 1, &1)
            .unwrap()
            .path;
        let second_path = persistence
            .persist(rustscan_gs::TrainingCheckpointReason::Periodic, 1, &2)
            .unwrap()
            .path;
    ...
    fn committed_checkpoint_reports_retention_failure_as_a_warning() {
        ...
        let outcome = persistence
            .persist_with_retention(
                rustscan_gs::TrainingCheckpointReason::Periodic,
                1,
                &1,
                |_| Err(io::Error::other("injected retention failure")),
            )
            .unwrap();
