...
cargo run --bin rustgs -- train \
  ...
  --output artifacts/runs/scene.ply \
  --iterations 30000
```
...
When `--output` is omitted, training writes `artifacts/runs/scene.ply`. The CLI still accepts an
...
cargo run --bin rustgs -- render \
  --input artifacts/runs/scene.ply \
  --camera camera.json \
  --output artifacts/runs/render.png
```
...
```sh
cargo fmt -p rustscan-gs -- --check
cargo test -p rustscan-gs --all-targets --no-fail-fast
cargo test -p rustscan-gs --no-default-features --all-targets --no-fail-fast
cargo clippy -p rustscan-gs --all-targets -- -D warnings
cargo test -p rustscan-gs --features rustsfm-contract-tests --lib \
  io::colmap_dataset::tests::loads_sparse_text_fixture_written_by_rustsfm
cargo test -p rustscan-gs --test integration_test -- --ignored
```
